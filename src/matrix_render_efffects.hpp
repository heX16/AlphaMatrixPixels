...
public:
    ...
    void renderTriangle(const csRect& target, float x1, float y1, float x2, float y2, float x3, float y3) const {
    ...
    void fillTriangleScanlineDirect(const csRect& target, 
                                     float xTop, float yTop,   // Top vertex
                                     float xMid, float yMid,   // Middle vertex (by Y)
                                     float xBot, float yBot) const {
        if (!matrix || target.empty()) {
    ...
    void render(csRandGen& /*rand*/, tTime /*currTime*/) const override {
        ...
        renderTriangle(target, x1, y1, x2, y2, x3, y3);
    }
};
